...
public class HtmlRootResponse implements RootResponse {
  ...
  public void assertHasTitle(String title) {
    WebElement titleElement = webDriver.findElement(By.className("jumbotron"))
  ...
  public RyvrsCollectionResponse followRyvrsLink() {
    ...
    for (int i = 0; i < 10 && !visible; ++i) {
      try {
        (new WebDriverWait(webDriver, 5)).until(ExpectedConditions.visibilityOfAllElements(links));
        visible = true;
